...
logical ext
integer dmount,smount,nsmount
integer do_n,nlen,year_b,year_e,nh,nnh,nseg
...
real sig02(nsmax),hilb(nsmax)
real sig12(nsmax),rsig1(nsmax),rsig2(nsmax)
real tsig1(nsmax),tsig2(nsmax)
            ...
            do id=begday,endday ! loop over day
               ...
               do ih=1,nh            ! loop over hour segment
                  ...
                  do iseg=1,nseg
                     ...
                     sachead1%npts=nsmp
                     sachead1%b=beg
                     sachead1%user0=real(smount)
                     if(doncc.eq.1)then
                        rsig1(1:nsmp)=sig11(1:nsmp)/depmax1+rsig1(1:nsmp) ! linear stack
                        if(dotl.eq.1)then
                           write(output,'(1a,"/tl_ncc_ns_",i5.5,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,".SAC")')&
                           trim(dirout),smount,net1(is),trim(sta1(is)),net2(is),trim(sta2(is)),com(ic1),com(ic2)
                           call write_sac(output,rsig1,sachead1,nerr)
                           if(nerr.eq.-1)stop 'Error in writing linear stack file'
                        endif
                        if(dotfpws.eq.1)then
                           write(output,'(1a,"/tf_ncc_ns_",i5.5,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,".SAC")')&
                           trim(dirout),smount,net1(is),trim(sta1(is)),net2(is),trim(sta2(is)),com(ic1),com(ic2)
                           call do_tfpws(sig11,nsmp,nn,n21,dt,ctf12,ctfps1)
                           do ns=1,nn
                              do nf=1,n21
                                 cdum=ctf12(ns,nf)/real(smount)
                                 rdum=cabs(ctfps1(ns,nf))/real(smount)
                        ...
                        if(dopws.eq.1)then
                           call do_pws(sig11,nsmp,dt,pw_sig1)
                           write(output,'(1a,"/pw_ncc_ns_",i5.5,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,".SAC")')&
                           trim(dirout),smount,net1(is),trim(sta1(is)),net2(is),trim(sta2(is)),com(ic1),com(ic2)
                           do i=1,nsmp 
                              tsig1(i)=(cabs(pw_sig1(i))/real(smount))**wu*rsig1(i)
                           enddo
                           call write_sac(output,rsig1,sachead1,nerr)
                     ...
                     if(dopcc.eq.1)then
                        rsig2(1:nsmp)=sig12(1:nsmp)/depmax2+rsig2(1:nsmp) ! linear stack
                        if(dotl.eq.1)then
                           write(output,'(1a,"/tl_pcc_ns_",i5.5,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,".SAC")')&
                           trim(dirout),smount,net1(is),trim(sta1(is)),net2(is),trim(sta2(is)),com(ic1),com(ic2)
                           call write_sac(output,rsig2,sachead1,nerr)
                           if(nerr.eq.-1)stop 'Error in writing linear stack file'
                        endif
                        if(dotfpws.eq.1)then
                           call do_tfpws(sig12,nsmp,nn,n21,dt,ctf22,ctfps2)
                           write(output,'(1a,"/tf_pcc_ns_",i5.5,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,".SAC")')&
                           trim(dirout),smount,net1(is),trim(sta1(is)),net2(is),trim(sta2(is)),com(ic1),com(ic2)
                           do ns=1,nn
                              do nf=1,n21
                                 cdum=ctf22(ns,nf)/real(smount)
                                 rdum=cabs(ctfps2(ns,nf))/real(smount)
                                 ctf(ns,nf)=cdum*(rdum**wu)
                              enddo
                           enddo
                           cdum=cmplx(0.0,-twpi/float(nn))
                        ...
                        if(dopws.eq.1)then
                           call do_pws(sig12,nsmp,dt,pw_sig2)
                           write(output,'(1a,"/pw_ncc_ns_",i5.5,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,".SAC")')&
                           trim(dirout),smount,net1(is),trim(sta1(is)),net2(is),trim(sta2(is)),com(ic1),com(ic2)
                           do i=1,nsmp 
                              tsig2(i)=(cabs(pw_sig2(i))/real(smount))**wu*rsig2(i)
                           enddo
                           call write_sac(output,tsig2,sachead1,nerr)
                           if(nerr.eq.-1)stop 'Error in writing pws file'
                        endif
                     endif
                  enddo        ! end loop over segments
               enddo           ! end loop over hour segments
               if(nsmount.gt.1)dmount=dmount+1
               sachead1%user0=real(dmount)
               if(nsmount.eq.0)cycle
               if(doncc.eq.1)then
                  if(dotl.eq.1)then
                     write(output,'(1a,"/tl_ncc_nd_",i5.5,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,".SAC")')&
                     trim(dirout),dmount,net1(is),trim(sta1(is)),net2(is),trim(sta2(is)),com(ic1),com(ic2)
                     call write_sac(output,rsig1,sachead1,nerr)
                     if(nerr.eq.-1)stop 'Error in writing linear stack file'
                  endif
                  if(dotfpws.eq.1)then
                     write(output,'(1a,"/tf_ncc_nd_",i5.5,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,".SAC")')&
                     trim(dirout),dmount,net1(is),trim(sta1(is)),net2(is),trim(sta2(is)),com(ic1),com(ic2)
                     do ns=1,nn
                        do nf=1,n21
                           cdum=ctf12(ns,nf)/real(smount)
                  ...
                  if(dopws.eq.1)then
                     call do_pws(sig11,nsmp,dt,pw_sig1)
                     write(output,'(1a,"/pw_ncc_nd_",i5.5,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,".SAC")')&
                     trim(dirout),dmount,net1(is),trim(sta1(is)),net2(is),trim(sta2(is)),com(ic1),com(ic2)
                     do i=1,nsmp 
                        tsig1(i)=(cabs(pw_sig1(i))/real(smount))**wu*rsig1(i)
                     enddo
               ...
               if(dopcc.eq.1)then  ! phase cross-correlation
                  if(dotl.eq.1)then
                     write(output,'(1a,"/tl_pcc_nd_",i5.5,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,".SAC")')&
                     trim(dirout),dmount,net1(is),trim(sta1(is)),net2(is),trim(sta2(is)),com(ic1),com(ic2)
                     call write_sac(output,rsig2,sachead1,nerr)
                     if(nerr.eq.-1)stop 'Error in writing linear stack file'
                  endif
                  if(dotfpws.eq.1)then
                     call do_tfpws(sig12,nsmp,nn,n21,dt,ctf22,ctfps2)
                     write(output,'(1a,"/tf_pcc_nd_",i5.5,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,".SAC")')&
                     trim(dirout),dmount,net1(is),trim(sta1(is)),net2(is),trim(sta2(is)),com(ic1),com(ic2)
                     do ns=1,nn
                        do nf=1,n21
                           cdum=ctf22(ns,nf)/real(smount)
                  ...
                  if(dopws.eq.1)then
                     write(output,'(1a,"/pw_pcc_nd_",i5.5,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,"_",1a,".SAC")')&
                     trim(dirout),dmount,net1(is),trim(sta1(is)),net2(is),trim(sta2(is)),com(ic1),com(ic2)
                     do i=1,nsmp 
                        tsig2(i)=(cabs(pw_sig2(i))/real(smount))**wu*rsig2(i)
                     enddo
